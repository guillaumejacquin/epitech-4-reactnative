...
const Repository = ({ route, navigation, octokit }) => {
  ...
  const getStarsRepo = async () => {
    var result = await octokit
      ...
      .then((res) => {
        if (res.status === 204) setstarsStatus(true);
      })
      .catch((e) => {
        setstarsStatus(false);
      });
  ...
  const getData = () => {
    ...
    octokit
      ...
      .then((res) => {
        setContributors(res.data?.length ? res.data.length : 0);
      });
      ...
      headerRight: () => (
        <TouchableOpacity style={{ fontSize: 16 }} onPress={getStars}>
          {starsStatus ? (
            ...
            <Image
              source={require("../../Image/unstars.png")}
              style={{ width: 15, height: 15 }}
            />
          )}
        </TouchableOpacity>
        ...
        <View style={{ flexDirection: "column", marginTop: 30 }}>
          <View style={{ flexDirection: "row", marginLeft: 20 }}>
            {imageUrl ? (
              <Image
                source={{ uri: imageUrl }}
                style={{ width: 72.53, height: 72.53, borderRadius: 72.53 / 2 }}
              />
            ) : null}
          ...
          {description ? (
            <Text
              ...
              ellipsizeMode={"tail"}
              onPress={() => {
                if (numberOfLines == 5) setNumberOfLines(0);
                else setNumberOfLines(5);
              }}
            >
              {description}
            </Text>
          ...
          >
            ...
            <Text style={styles.infos}>{repo.default_branch}</Text>
          </View>
          <View
            style={{
              flexDirection: "row",
          ...
          >
            ...
            <Text style={styles.infos}>{stars}</Text>
          </View>

          <View style={{ marginVertical: 20 }}>
            <TouchableOpacity
              onPress={() => {
                navigation.navigate("Repository browser", { repo: repo });
              }}
            >
            ...
            <TouchableOpacity
              onPress={() => {
                navigation.navigate("Pull requests", { repo: repo });
              }}
            >
              <View style={styles.statBar}>
            ...
            <TouchableOpacity
              onPress={() => {
                navigation.navigate("Issues", { repo: repo });
              }}
            >
              <View style={styles.statBar}>
            ...
            <TouchableOpacity
              onPress={() => {
                navigation.navigate("Forks", { repo: repo });
              }}
            >
              <View style={styles.statBar}>
            ...
            <TouchableOpacity
              onPress={() => {
                navigation.navigate("Watchers", { repo: repo });
              }}
            >
              <View style={styles.statBar}>
            ...
            <TouchableOpacity
              onPress={() => {
                navigation.navigate("Contributors", { repo: repo });
              }}
            >
              <View style={styles.statBar}>
          ...
          {readme ? <Text style={styles.descriptionText}>{readme}</Text> : null}
          {userName != repo.owner.login ? null : (
            <TouchableOpacity
              onPress={() => {
                deleteRepository();
              }}
